# 07 — Tela de Perfil do Jogador

Exibe estatísticas, rating atual, histórico de partidas e permite editar o nome ou ajustar o rating manualmente.

---

## Componente (`src/pages/Player/PlayerProfile.jsx`)
...
export default function PlayerProfile() {
  ...
  // (simplificado — em produção, indexar por playerId)
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="bg-sage-light border-b border-sage px-4 py-5 text-center">
        <div className="w-16 h-16 rounded-full bg-sage text-sage-dark
                        ...
                        mx-auto mb-2.5 border-3 border-white shadow-sm">
          {initials}
        </div>
...
O perfil do jogador pode ser acessado de três lugares:

1. **Na tela de check-in** — toque longo em um jogador abre o perfil
2. **Na tela de partida** — toque no nome de um jogador em campo
3. **Via rota direta** — `/player/:playerId`

Para implementar o toque longo no check-in, adicionar ao `PlayerRow`:

```jsx
// Em PlayerRow.jsx, adicionar prop onLongPress e handler
<div
  onClick={onToggle}
  onContextMenu={e => { e.preventDefault(); onLongPress?.() }}
  // Para mobile: usar onTouchStart + onTouchEnd com setTimeout
  ...
>
```

Implementação simples do long press para mobile:
...
function useLongPress(callback, delay = 500) {
  ...
  return {
    onTouchStart: () => { timer.current = setTimeout(callback, delay) },
