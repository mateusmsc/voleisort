export default function FieldTeams({ teamA, teamB, onEditCurrent, onShuffle, showLevels, onToggleLevels, winStreak = 0 }) {
  const hasActions = Boolean(onEditCurrent || onShuffle || onToggleLevels)
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <MiniTeamBox label="Time A" color="sage" players={teamA} showLevels={showLevels} winStreak={winStreak} />
...
function MiniTeamBox({ label, color, players, showLevels, winStreak = 0 }) {
  const styles = {
    sage: {
  ...
  return (
    <div className={`rounded-xl border-2 ${s.border} ${s.bg} p-3`}>
      <div className="flex items-center justify-between mb-2">
        ...
        {winStreak >= 2 && (
          <span className="text-xs font-medium bg-amber-100 dark:bg-amber-800/60
                           text-amber-700 dark:text-amber-300 border border-amber-300 dark:border-amber-700
